...
Observe that the 16k byte (E)(E)(P)ROM in the cartridge is mapped 
to 4 memory pages of each 4k byte: from 0x1000 to 0x4FFF. 
This poses an addressing problem. If the Z80 puts 0x1000 on the bus,
...
I was very confused by this, but the 
[Simple Cartridge from Ivo Filot](https://philips-p2000t.nl/cartridges/simple-cartridge.html) 
explains how this is done in practice.

The problem at hand is that we need to compute the address lines 
_A12C_ and _A13C_ for the cartridge from the bus lines _A12_ and _A12_ 
(and signal lines _CARS1_ and _CARS2_). Answer: _A12C_ is _not(A12)_ and 
_A13C_ is the cartridge select signal _CARS2_. See the orange column below.

![Cartridge address mapping](images/cartridgeaddresmap.png)

We still need a cartridge select signal CARS; this is simply the OR of CARS1 
...
- [Preservation project](https://github.com/p2000t), especially 
  [Nieuwsbrief Natlab](https://github.com/p2000t/documentation/tree/main/NatLab) and 
